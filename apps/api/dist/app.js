...
const demoSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600" viewBox="0 0 800 600"><defs><linearGradient id="sky" x2="0" y2="1"><stop stop-color="#b6cce7"/><stop offset="1" stop-color="#f3dfc1"/></linearGradient></defs><rect width="800" height="600" fill="url(#sky)"/><circle cx="590" cy="165" r="64" fill="#fff3c4"/><path d="M0 480L260 160 550 600H0" fill="#526f69"/><path d="M210 600L555 260 800 520V600" fill="#73968b"/><path d="M0 525Q260 475 800 550V600H0" fill="#c4d2c2"/></svg>';
export async function buildApp(options = {}) {
    ...
    app.addContentTypeParser('application/json', { parseAs: 'buffer' }, (request, body, done) => {
        ...
        request.rawJson = body.toString('utf8');
        try {
    ...
    const allow = (url) => {
        ...
        const supported = ['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'TRACE'];
    ...
    await app.register(swagger, {
        openapi: {
            ...
            info: {
                title: 'Canvas API',
                version: '1.0.0',
                description: 'Локальный API для рабочего пространства с нодами. Авторизация не нужна. GET графа возвращает ETag; PUT требует If-Match. Ответы содержат сам ресурс, без обёртки data. Генерация имитируется.',
            },
            servers: [{ url: '/' }],
    ...
    app.setNotFoundHandler((_request, reply) => reply.code(404).send({ error: { code: 'ROUTE_NOT_FOUND', message: 'Маршрут не найден.' } }));
    ...
    add('GET', '/api/config', { id: 'getConfig', summary: 'Параметры тестового', tag: 'API', data: C.Config }, async () => ({
        debounceMs: 500,
        pollIntervalMs: 500,
    ...
    add('GET', '/api/spaces', { id: 'listSpaces', summary: 'Рабочие пространства', tag: 'Spaces', data: Type.Array(C.Space) }, async () => store.spaces());
    add('POST', '/api/spaces', {
        id: 'createSpace',
        ...
        const space = store.createSpace(r.body.title);
        return p.header('Location', space.links.self.href).code(201).send(space);
    });
    add('GET', '/api/spaces/:spaceId', {
        id: 'getSpace',
    ...
    add('GET', '/api/spaces/:spaceId/graph', {
        id: 'getGraph',
        ...
        description: 'Сохранённые nodes, edges и viewport. If-None-Match позволяет получить 304 без тела. Сильный ETag относится только к этому ресурсу.',
    }, (r, p) => {
        ...
        if (r.headers['if-match'] !== undefined &&
            !matchesETag(r.headers['if-match'], graph.etag))
    ...
    add('PUT', '/api/spaces/:spaceId/graph', {
        id: 'replaceGraph',
        ...
        description: 'Передайте весь граф и If-Match из предыдущего GET/PUT. Ответ содержит тот же JSON и новый ETag. Идентичный повтор с актуальным ETag не меняет данные.',
    }, (r, p) => {
        if (r.headers['if-none-match'] !== undefined) {
            const graph = store.graph(r.params.spaceId);
            if (r.headers['if-match'] === undefined)
                throw new DomainError(428, 'PRECONDITION_REQUIRED', 'Передайте If-Match.');
            if (!matchesETag(r.headers['if-match'], graph.etag))
                throw new DomainError(412, 'GRAPH_VERSION_CONFLICT', 'Граф изменился.');
            if (matchesETag(r.headers['if-none-match'], graph.etag, true))
                throw new DomainError(412, 'GRAPH_VERSION_CONFLICT', 'Условие сохранения не выполнено.');
        }
        const graph = store.saveGraph(r.params.spaceId, r.rawJson, r.headers['if-match']);
        return p.header('ETag', graph.etag).type('application/json; charset=utf-8').send(graph.raw);
    });
    add('GET', '/api/spaces/:spaceId/generations', {
        id: 'listGenerations',
    ...
    add('POST', '/api/spaces/:spaceId/generations', {
        id: 'createGeneration',
        ...
        description: 'Сначала завершите сохранение графа. graphETag в теле указывает снимок графа; If-Match здесь не используется. processing: 202 с Location и Retry-After; мгновенное создание: 201; завершённый повтор по ключу: 200.',
    }, (r, p) => {
        const value = store.createGeneration(r.params.spaceId, r.body, r.headers['idempotency-key']);
        const status = value.data.status === 'processing' ? 202 : value.created ? 201 : 200;
        if (status !== 200)
            p.header('Location', value.data.links.self.href);
        if (status === 202)
            p.header('Retry-After', '1');
        return p.code(status).send(value.data);
    });
    add('GET', '/api/spaces/:spaceId/generations/:generationId', {
        id: 'getGeneration',
    ...
    }, (_r, p) => p.header('Cache-Control', 'public, max-age=3600').type('image/svg+xml').send(demoSvg));
    await app.ready();
    return app;
}
