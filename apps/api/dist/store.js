...
export const etag = (raw) => `"${createHash('sha256').update(raw).digest('hex')}"`;
export class Store {
    file;
    delayMs;
    now;
    state;
    constructor(file, delayMs = 1500, now = Date.now) {
        this.file = file;
    ...
    storedSpace(id) {
        ...
        if (!space)
            throw new DomainError(404, 'SPACE_NOT_FOUND', 'Рабочее пространство не найдено.');
        return space;
    }
    space(id) {
    ...
    graph(id) {
        ...
        return { raw, etag: etag(raw), data: JSON.parse(raw) };
    }
    saveGraph(id, raw, condition) {
        const previous = this.graph(id);
        if (condition === undefined)
            throw new DomainError(428, 'PRECONDITION_REQUIRED', 'Передайте If-Match с ETag загруженного графа.');
        if (!matchesETag(condition, previous.etag))
            throw new DomainError(412, 'GRAPH_VERSION_CONFLICT', 'Граф изменился. Сохраните свои правки и перечитайте данные сервера.');
    ...
    validateGraph(graph) {
        ...
        for (const edge of graph.edges) {
            ...
            if (!valid ||
                ...
                throw new DomainError(422, 'INVALID_GRAPH', 'Соединяйте текст с генератором, генератор с результатом. У входа одна связь, у генератора один результат.');
            ids.add(edge.id);
            targets.add(edge.target);
            if (source.type === 'generator')
                outputs.add(source.id);
        }
    }
    generation(spaceId, id) {
        ...
        if (!record || record.spaceId !== spaceId)
            throw new DomainError(404, 'GENERATION_NOT_FOUND', 'Генерация не найдена в этом пространстве.');
        const { settlesAt, ...value } = record;
        const status = this.now() < settlesAt
            ? 'processing'
    ...
    createGeneration(spaceId, body, key) {
        this.storedSpace(spaceId);
        ...
        if (previous) {
            if (previous.fingerprint !== fingerprint)
                throw new DomainError(409, 'IDEMPOTENCY_CONFLICT', 'Ключ уже использован с другими данными.');
        ...
        if (body.graphETag !== graph.etag)
            throw new DomainError(409, 'GRAPH_CHANGED', 'Сначала сохраните текущий граф и используйте его новый ETag.');
        ...
        if (!output || prompt?.type !== 'prompt' || !prompt.data.text.trim())
            throw new DomainError(422, 'INCOMPLETE_CHAIN', 'Соедините непустой текст, генератор и результат.');
        if (this.generations(spaceId).some((g) => g.nodeId === node.id && g.status === 'processing'))
            throw new DomainError(409, 'GENERATION_IN_PROGRESS', 'У этой ноды уже есть активная генерация.');
        return this.commit(() => {
            const id = randomUUID();
            this.state.generations[id] = {
                id,
